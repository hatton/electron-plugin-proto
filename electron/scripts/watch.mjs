...
//   mode,
//   logLevel,
//   configFile: 'packages/plugin/vite.config.js',
// }).then(s => s.listen());
...
const rendererWatchServer = await createServer({
  ...
  // reload the render if we change the plugin
  const pluginPath = path.join(process.cwd(), '../plugins/hello/dist/hello-plugin.js');
  console.log('**** looking for plugin at ' + pluginPath);
  s.watcher.add(pluginPath);
  s.watcher.on('change', () => {
    s.ws.send({
      type: 'full-reload',
    });
